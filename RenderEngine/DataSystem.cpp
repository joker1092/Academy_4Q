...
void DataSystem::RenderForEditer()
{
	...
	// ÌÉÄÏùºÏùò ÌÅ¨Í∏∞ÏôÄ Í∞ÑÍ≤© ÏÑ§Ï†ï

	ImGui::ContextRegister("Models", [&]()
	{
		constexpr float tileSize = 64.0f;  // ÌÉÄÏùº ÌÅ¨Í∏∞ (ÏïÑÏù¥ÏΩò ÌÅ¨Í∏∞)
		constexpr float padding = 10.0f;  // ÌÉÄÏùº Í∞Ñ Í∞ÑÍ≤©
		constexpr int tilesPerRow = 4;    // Ìïú Ï§ÑÏóê ÌëúÏãúÌï† ÌÉÄÏùº Ïàò
		int count = 0;
		...
		for (const auto& [key, model] : Models)
		{
			...
			{
				//∏∏æ‡ø° Ω√∞£¿Ã ≥≤æ∆µπæ∆º≠ Guizmo∏¶ æ¥¥Ÿ∏È ø©±‚º≠ ∏µ®¿ª µÂ∑°±◊æÿµÂ∑”«ÿº≠ æ¿¿∏∑Œ ∫∏≥æ ºˆ ¿÷∞‘ «ÿæﬂ«‘.
	...
	//Ïù¥Í±¥ ÎÇòÏ§ëÏóê Í≤åÏûÑ Ïò§Î∏åÏ†ùÌä∏Î°ú Í∞ÄÎçòÏßÄ Ìï¥ÏïºÎê®.
