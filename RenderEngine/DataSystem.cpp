...
void DataSystem::RenderForEditer()
{
	...
	{
		constexpr float tileSize = 64.0f;  // ?Ä???¨Í∏∞ (?ÑÏù¥ÏΩ??¨Í∏∞)
		constexpr float padding = 10.0f;  // ?Ä??Í∞?Í∞ÑÍ≤©
		constexpr int tilesPerRow = 4;    // ??Ï§ÑÏóê ?úÏãú???Ä????
		int count = 0;
	...
	//?¥Í±¥ ?òÏ§ë??Í≤åÏûÑ ?§Î∏å?ùÌä∏Î°?Í∞Ä?òÏ? ?¥Ïïº??
