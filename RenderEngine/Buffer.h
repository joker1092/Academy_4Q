...
{
    IBuffer() = default;
    IBuffer(uint32 access, D3D11_USAGE usage, D3D11_BIND_FLAG flags) : m_Access(access), m_Usage(usage), m_Flags(flags) {}
...
public:
    ...
    unsafe D3D11_MAPPED_SUBRESOURCE Map(D3D11_MAP mapType, uint32 subResource = 0)
    {
        ...
        DirectX11::ThrowIfFailed(m_pContext->Map(m_Buffer.Get(), subResource, static_cast<D3D11_MAP>(m_Access), NULL, &mappedResource));
    ...
    {
#if defined(_DEBUG)
        CORE_ASSERT(buffer.GetUsage() == D3D11_USAGE::D3D11_USAGE_STAGING);
        CORE_ASSERT(_useage != D3D11_USAGE::D3D11_USAGE_IMMUTABLE);
#endif
