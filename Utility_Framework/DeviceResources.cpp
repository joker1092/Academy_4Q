...
{
	if (m_logicalSize.width != logicalSize.width or m_logicalSize.height != logicalSize.height)
	{
...
void DirectX11::DeviceResources::UpdateRenderTargetSize()
{
	...
	if (!DisplayMetrics::SupportHighResolutions and m_dpi > DisplayMetrics::DpiThreshold)
	{
		...
		if (max(width, height) > DisplayMetrics::WidthThreshold and min(width, height) > DisplayMetrics::HeightThreshold)
		{
			m_effectiveDpi /= 2.0f;
		}
	}
